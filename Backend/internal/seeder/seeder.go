...
var seedClient = &http.Client{Timeout: 15 * time.Second}

func doGet(apiURL string) ([]byte, error) {
...
// AutoSeed seeds the database with manga data if it is empty.
// Called automatically at startup so the production DB is populated.
func AutoSeed(db *sql.DB) {
	var count int
	db.QueryRow("SELECT COUNT(*) FROM manga").Scan(&count)
	if count > 0 {
		log.Printf("📚 Database already has %d manga — skipping auto-seed", count)
		return
	}

	log.Println("🌱 Empty database detected — running auto-seed...")
	seedManual(db)
	seedMangaDex(db)
	db.QueryRow("SELECT COUNT(*) FROM manga").Scan(&count)
	log.Printf("✅ Auto-seed complete — %d manga in database", count)
}

// ── Manual seed (35 well-known manga) ────────────────────────────────────
...
func seedManual(db *sql.DB) {
	entries := []mangaEntry{
		...
		{ID: "josei-wotakoi", Title: "Wotakoi Love is Hard for Otaku", Author: "Fujita", Genres: []string{"Romance", "Comedy", "Josei"}, Status: "completed", TotalChapters: 60, Description: "Two otaku adults start dating after reuniting at their workplace."},
...
func fetchCoverURLByTitle(title string) string {
	time.Sleep(300 * time.Millisecond)
	apiURL := fmt.Sprintf(
		"https://api.mangadex.org/manga?title=%s&limit=1&includes[]=cover_art",
	...
	for _, rel := range m.Relationships {
		if rel.Type == "cover_art" && rel.Attributes != nil {
			if fileName, ok := rel.Attributes["fileName"].(string); ok && fileName != "" {
				return fmt.Sprintf("https://uploads.mangadex.org/covers/%s/%s.256.jpg", m.ID, fileName)
			}
		}
	}
	return ""
...
func seedMangaDex(db *sql.DB) {
	...
	for _, offset := range offsets {
		apiURL := fmt.Sprintf(
			"https://api.mangadex.org/manga?limit=25&offset=%d&contentRating[]=safe&contentRating[]=suggestive&includes[]=cover_art",
			offset,
		)
		body, err := doGet(apiURL)
		if err != nil {
			log.Printf("  ⚠ MangaDex fetch failed (offset %d): %v", offset, err)
			continue
		}
		...
		for _, m := range result.Data {
			...
			for _, rel := range m.Relationships {
				if rel.Type == "cover_art" && rel.Attributes != nil {
					if fileName, ok := rel.Attributes["fileName"].(string); ok && fileName != "" {
						coverURL = fmt.Sprintf("https://uploads.mangadex.org/covers/%s/%s.256.jpg", m.ID, fileName)
						break
					}
				}
			}

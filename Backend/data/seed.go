...
type Manga struct {
	...
	Description   string
	CoverURL      string
	MangaDexQuery string // search term to find cover
}

type MDexResp struct {
	Data []struct {
		ID            string `json:"id"`
...
func fetchCoverURL(query string) string {
	time.Sleep(300 * time.Millisecond) // respect rate limit
	apiURL := fmt.Sprintf(
	...
	if err != nil {
		log.Printf("  ⚠ cover fetch failed for %q: %v", query, err)
		return ""
	}
...
func main() {
	...
	log.Println("🔍 Fetching cover URLs from MangaDex API...")
	for i := range mangas {
		coverURL := fetchCoverURL(mangas[i].Title)
		mangas[i].CoverURL = coverURL
		if coverURL != "" {
			log.Printf("  🖼  %s → got cover", mangas[i].Title)
		} else {
			log.Printf("  ⚠  %s → no cover found", mangas[i].Title)
		}
	}
	...
	log.Println("🎉 Seeding complete! 35 manga added with covers.")
}
